...
class ExpSMOutcomeFeedback:
    ...
    def run(
        ...
        for decision_event in self._direct_mechanism_decisions(tick, memory):
            decision = dict(decision_event.payload)
            decision_id = str(decision.get("decision_id", ""))
            experience_id = str(decision.get("source_experience_id", ""))
            ...
            if outcome_event is None:
                continue
            activation = {
                "activation_id": decision.get("source_mechanism_search_id"),
                "experience_id": experience_id,
                "then_patterns": [decision.get("decision_pattern_id")],
                "result_patterns": [decision.get("source_target_pattern_id")],
            ...
            if status == "no_feedback":
                continue
            key = (experience_id, str(decision.get("source_mechanism_search_id", "")), decision_id)
            ...
            operation = self._apply_feedback(
                ...
                status,
                key,
                "direct_decision_mechanism_source",
            )
    ...
    def _direct_mechanism_decisions(self, tick: int, memory: ContextMemory) -> list[ContextOperation]:
        return [
            event
            for event in memory.events
            if event.marker == OperationMarker.INTERNAL_DECISION
            and event.payload.get("source") == "expsm_mechanism_search"
            and event.payload.get("source_experience_id")
            and event.payload.get("source_mechanism_search_id")
            and tick - event.tick >= FEEDBACK_DELAY_TICKS
        ]
    ...
    def _selected_decisions(
        ...
        for event in memory.events:
            ...
            if payload.get("source") == "expsm_mechanism_search":
                continue
