...
class DraftContextRelevanceScorer:
    ...
    def score_if_patterns(
        ...
        core_family = _core_family(review_payload, self.pattern_registry)
        ...
        for pattern_id in _unique([str(pattern) for pattern in candidate_patterns if pattern]):
            ...
            if _is_technical_pattern(debug_name):
                records.append(
                    {
                        "pattern": pattern_id,
                        "score": 0.0,
            ...
            family_match = _matches_core_family(debug_name, core_family)
            if family_match:
                score += 0.30
            ...
            if _different_modality_without_link(debug_name, core_family, evidence):
                score -= 0.10
                reasons.append("different_modality_without_link")
            if _competing_family_pattern(debug_name, core_family):
                score -= 0.20
                reasons.append("competing_action_family_penalty")
            if debug_name in {"internal_learning_candidate", "prediction_future_state"}:
                score -= 0.05
...
def _core_family(review_payload: dict[str, Any], pattern_registry: PatternRegistry) -> str:
    core_chain = review_payload.get("core_chain", {})
    names = []
    for key in ("decision_patterns", "effect_patterns"):
        names.extend(pattern_registry.debug_name(str(pattern)) for pattern in core_chain.get(key, ()) if pattern)
    joined = " ".join(names)
    return joined


def _matches_core_family(debug_name: str, core_family: str) -> bool:
    if "action_preserve_integrity" in core_family or "state_integrity_preservation" in core_family:
        return debug_name in {
            "tone_stability_low",
            "tone_integrity_low",
    ...
    if "action_reduce_load" in core_family or "state_load_reduced" in core_family:
        return debug_name in {
            "tone_fatigue_high",
            "tone_tension_high",
            "sen_memory_pressure",
    ...
    if "action_increase_attention" in core_family or "state_attention_increased" in core_family:
        return debug_name in {
    ...
    if "action_inspect_pattern" in core_family or "state_pattern_inspection" in core_family:
        return debug_name in {
            "novel_activation_pattern",
            "known_memory_pattern",
            "related_memory_pattern",
            "label_novelty",
        }
    return False


def _different_modality_without_link(debug_name: str, core_family: str, evidence: _Evidence) -> bool:
    del evidence
    if ("action_preserve_integrity" in core_family or "state_integrity_preservation" in core_family) and "attention_visual" in debug_name:
        return True
    if ("action_reduce_load" in core_family or "state_load_reduced" in core_family) and debug_name.startswith("img_"):
        return True
    return False


def _competing_family_pattern(debug_name: str, core_family: str) -> bool:
    attention_context = {
        "periodic_audio_pattern",
        "prediction_future_state",
        "internal_attention_audio",
    ...
    if "action_preserve_integrity" in core_family or "state_integrity_preservation" in core_family:
        return debug_name in attention_context
    if "action_increase_attention" in core_family or "state_attention_increased" in core_family:
        return debug_name in integrity_context
    return False
...
def _contains_confirmed_outcome(review_payload: dict[str, Any], pattern_registry: PatternRegistry) -> bool:
    ...
    if not isinstance(core_chain, dict):
        return False
    return any(pattern_registry.debug_name(str(pattern)) == "outcome_confirmed" for pattern in core_chain.get("outcome_patterns", ()))
...
def _is_technical_pattern(debug_name: str) -> bool:
    excluded_exact = {
