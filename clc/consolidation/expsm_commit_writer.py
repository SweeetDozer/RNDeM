...
class ExpSMCommitWriter:
        ...
        if not self.memory_mutation_policy.allow_expsm_commit:
            return [
                self._policy_blocked_update(
                    tick,
                    "expsm_commit_blocked_by_policy",
                    "policy_disallows_expsm_commit",
                )
            ]
        try:
    ...
    def _has_technical_if_patterns(self, draft: dict[str, Any]) -> bool:
        return any(is_memory_write_technical_pattern(self.pattern_registry, str(pattern)) for pattern in draft.get("if_patterns", ()))
