...
from clc.action.candidate_sources import is_expsm_activation_source
...
MAX_DECISION_AUDIT_CANDIDATES = 9
...
class DecisionSelector:
    ...
    def select(
        ...
        payload["decision_candidate_audit_snapshot"] = _decision_candidate_audit_snapshot(scored, candidate)
        if mode_action_guard is not None:
            payload["guard_candidate_audit_snapshot"] = mode_action_guard.recent_candidate_audit(tick, limit=20)
...
def _decision_candidate_audit_snapshot(
    scored: list[tuple[ActionCandidate, dict[str, float]]],
    selected: ActionCandidate,
) -> list[dict[str, object]]:
    snapshot: list[dict[str, object]] = []
    ranked = sorted(scored, key=lambda item: item[1]["final_score"], reverse=True)
    for candidate, breakdown in ranked[:MAX_DECISION_AUDIT_CANDIDATES]:
...
def _expsm_candidate_snapshot(
    ...
    for candidate, breakdown in scored:
        ...
        if not is_expsm_activation_source(source):
            continue
        snapshot.append(
            {
                "candidate_id": candidate.candidate_id,
