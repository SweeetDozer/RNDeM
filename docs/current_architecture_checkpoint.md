...
- Negative/no-behavior diagnostic wiring coverage exists. It verifies missing/
  ...
  `_run_tick()`.
- Tick diagnostic visibility ADR exists, but it is design-only. The preferred
  future shape is external diagnostic wrapper/harness, direct `_run_tick()`
  diagnostic hook is deferred, no runtime source code changed, no `_run_tick()`
  wiring exists, diagnostics remain unwired from behavior paths, no real
  ContextMemory reads/writes exist, and AKBSM writes remain blocked.
- External tick diagnostic wrapper scaffold exists, but it is explicit harness
  only. It stays outside `_run_tick()`, runs a provided tick callable without
  diagnostic inputs, preserves behavior output, returns diagnostic snapshot data
...
- Temporary metadata architecture map exists in
  `docs/contextmemory_temporary_metadata_architecture_map.md`. It documents the
  ladder from AKBSM proposal lifecycle metadata through local temporary
  placement, read-only observation, runtime diagnostics, and the external tick
...
- v1 readiness criteria exist in `docs/v1_readiness_criteria.md`. They define
  `v1.0.0` as a stable safety-bounded prototype checkpoint, not AGI or
